...
export default function QuestionView(props: any) {
  ...
  // const [user, setUser] = useState({});
    ...
    const handleNextQuestion = () => {
      socket.emit('nextQuestionServer', {roomCode, answerTime, questionNumber: questionNumber + 1});
      setUsers([]);
    ...
    return (
        ...
        <h2 className="white">{`ROOM CODE: ${roomCode}`  }</h2>    
         {users.map((userObject: any) => { 
            return (
               <Row key={userObject.userName} userName={userObject.userName} answer={userObject.answer}/>
            );
          })}   
        ...
        <MyButton className="next-question" onClick={() => handleNextQuestion()} variant="contained" color="primary">
            Next question
        </MyButton> 
        <Drawer/>
        </div>
