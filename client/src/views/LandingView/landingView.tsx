...
export default function App() {
  ...
  },[hasroomCodeError]); //only re-run the effect if new message comes in
  ...
  useEffect(() => {
    socket.on('validRoomCode', (data:any) => {
      ...
      history.push({pathname: Paths.question, state: { userName: data.userName, roomCode: data.roomCode }});
    });
    },[hasroomCodeError]); //only re-run the effect if new message comes in
...
const handleCreateRoom = async () => {
  socket.emit('createRoom', null,  function(roomCode: any){
    history.push({pathname: Paths.admin, state: { roomCode }});
    })
  }
...
const handleRoomCodeChange = (event: any) => {
  ...
  return (
    <div className="App">
      <MyButton onClick={() => handleCreateRoom()} variant="contained" color="primary">
        Create room
     </MyButton>
      <div id="Slider" className="slideup">
       <CustomTextField
        ...
        helperText={hasUserNameError ? "Incorrect entry or username already exists" : ""}
      ...
      <MyButton onClick={() => handleJoinRoom()} variant="contained" color="primary">
          Join room
      </MyButton> 
    </div>
  );
