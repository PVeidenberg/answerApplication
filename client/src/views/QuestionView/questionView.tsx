...
export default function QuestionView(props: any) {
    ...
    const handleAnswerSending = () => {
      console.log("sendAnswer", userName, answer, roomCode);
        ...
        socket.emit('sendAnswer', { userName, answer, roomCode }, (error: Boolean) => {
          if (error) {
            console.log(error);
          }
        })
    ...
    return (
        <div className="App">
        <h2 className="question-heading white" >{questionNumber == 0 ? "": `Question ${questionNumber}`}</h2>
        <CustomTextField
          ...
          variant="outlined"
        />
        <h4 className="white" >{`Time left : ${questionTimeLeft !== null ? questionTimeLeft : "0"}`}</h4>
        <MyButton onClick={() => handleAnswerSending()} disabled={isSendButtonDisabled} variant="contained" color="primary">
            Send your answer
        </MyButton> 
        </div>
    );
